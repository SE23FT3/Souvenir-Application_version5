...
public class ProductManager {
	...
	public boolean addProduct(String productCode, String productName,
			...
			float productPrice, int barCode, int reorderThreshold,
			int orderQuantity) throws IOException {

		boolean valid = false;
		Product p = new Product(createProductId(productCode), productName,
				productDescription, quantityAvaliable, productPrice, barCode,
	...
	public Product modifyProduct(int productNum, String productName,
		...
		if (p != null) {
			p = new Product(p.getProductId(), productName, productDescription,
					quantityAvaliable, productPrice, p.getBarCode(),
					reorderThreshold, orderQuantity);
		}
	...
	public ArrayList<Product> retrieveProductDataFromFile() throws IOException {
		...
		try {
			...
			while ((dataofFile = br.readLine()) != null) {
				...
				List<String> productString = Arrays.asList(dataofFile
						.split(","));
				for (int z = 0; z <= productString.size(); z++) {
					...
					product.setQuantityAvaliable(Integer.parseInt(productString
							.get(3)));
					product.setProductPrice(new Double(productString.get(4)));
					product.setBarCode(Integer.parseInt(productString.get(5)));
					product.setReorderThreshold(Integer.parseInt(productString
							.get(6)));
					product.setOrderQuantity(Integer.parseInt(productString
							.get(7)));
	...
	public ArrayList<Product> deleteProductData(Product product)
			throws IOException {
		ArrayList<Product> productList = new ArrayList<Product>();

...
public class LoginManager {
	...
	public ArrayList<StoreKeeper> retrieveStoreKeeperDataFromFile()
			throws IOException {
		...
		try {
			...
			while ((dataofFile = br.readLine()) != null) {
				...
				List<String> storeKeeperString = Arrays.asList(dataofFile
						.split(","));
	...
	public boolean validatelogin(String uname, String pwd) throws IOException {
		...
		while (i.hasNext()) {
			...
			if ((uname.equalsIgnoreCase(dis.getUsername().toString()))
					&& (pwd.equalsIgnoreCase(dis.getPassword().toString()))) {
				result = true;
			}
			j++;
			break;
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (part1.equalsIgnoreCase(uname)) {
					...
					JOptionPane
							.showMessageDialog(null, "Cannot find the user!");
				}

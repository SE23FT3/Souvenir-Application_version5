...
public class DiscountManager implements Constants{
	...
	public DiscountManager(MainMenu mainMenu) throws IOException {
	...
	public ArrayList<Discount> retrieveDiscountDataFromFile() throws IOException{
		...
		try {
			...
			while((dataofFile=br.readLine())!=null){
				...
				List<String> discountString = Arrays.asList(dataofFile.split(","));
	...
	public void displayDiscountFile(ArrayList discountList)
	{
		...
		while (iterator.hasNext()) {
			...
			System.out.println("Duration Of Discount:"+d.getDurationOfDiscount());
			System.out.println("Discount Percentage:"+d.getDiscountPercentage());
			System.out.println("Discount Applicability:"+d.getApplicability());
	...
	public boolean addNewDiscountData(Discount discount) throws IOException {
		...
		if(discount!=null)
		{
			...
			String content=discount.getDiscountCode()+","+discount.getDiscountDescription()+","+discount.getStartDate()+","+
					discount.getDurationOfDiscount()+","+discount.getDiscountPercentage()+","+discount.getApplicability();
			System.out.println(content);
		
	        BufferedWriter bout = new BufferedWriter(new FileWriter(Constants.DISCOUNTFILE,true));
	...
	public ArrayList<Discount> deleteDiscountData(Discount discount) throws IOException
	{
		...
		if(discount!=null)
		{
	      ...
	      //  displayDiscountFile(discountList);
			...
			while (iterator.hasNext()) {
				...
				if(dis.getDiscountCode().equalsIgnoreCase(discount.getDiscountCode()))
				{
	...
	public void refresh() throws IOException {
		...
		mainMenu.displayDiscountPanel();
		
	}
	public String getDiscountApplicable(String memberID) throws ParseException, IOException {
		// TODO Auto-generated method stub
			...
			while (iterator.hasNext()) {
				...
				if(memberID.equalsIgnoreCase("Public"))
				{
					...
					if(discount.getApplicability().equalsIgnoreCase("A"))
					{
						...
						if(!startDate.equalsIgnoreCase("Always") || (!duration.equalsIgnoreCase("Always") )){
							Date now=new Date();
							...
							if(startdateofDiscount.before(now))
							{
								...
								int differnece = getDifferenceDays(startdateofDiscount, now);
						...
						System.out.println("discountPerctList"+discountPerctList.size());
